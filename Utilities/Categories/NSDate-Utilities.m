...
+ (NSArray *)fullWeekDayNames {
    ...
    NSDate *date = [[NSDate date] firstDayOfWeek];
    for (int i = 1; i <= D_DAY_IN_WEEK; i++) {
        [ar addObject:[dateFormatter stringFromDate:[date dateByAddingDays:i]]];
    }
    return ar;
}
...
+ (NSArray *)superShortWeekDayNames {
    ...
    NSDate *date = [[NSDate date] firstDayOfWeek];
    for (int i = 1; i <= D_DAY_IN_WEEK; i++) {
        NSString *weekDay = [dateFormatter stringFromDate:[date dateByAddingDays:i]];
        [ar addObject:[weekDay substringToIndex:1]];
    }
    return ar;
}
...
- (NSDate *)dateByAddingYears:(NSInteger)years {
    ...
    comp.year = years;
    return [gregorian dateByAddingComponents:comp toDate:self options:0];
}
...
- (NSDate *)firstDayOfWeek {
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents* components = [gregorian components:(NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekday) fromDate:self];
...
- (NSDate *)firstDayOfMonth {
    NSCalendar *gregorian = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents* components = [gregorian components:DATE_COMPONENTS fromDate:self];
    components.day = 1;
    return [gregorian dateFromComponents:components];
}
...
- (NSInteger)daysInMonth {
    NSCalendar* gregorianCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSRange days = [gregorianCalendar rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:self];
    return days.length;
}
...
- (NSDateComponents *)dateComponents:(NSCalendarUnit)unitFlags {
    return [CURRENT_CALENDAR components:unitFlags fromDate:self];
}
